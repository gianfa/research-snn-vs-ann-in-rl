...
RUN_PREFIX = f'16-trial-r2-d1-40-clippedI-spenc'

exp_outdata_dir = EXP_DATA_DIR/'experiments'
data_path = EXP_DATA_DIR/"ieeg-toy-ds-spenc.pkl"
description = "Performance Sampling vs radius and degree"

dataset_is_spike_encoded = True
n_samples = 100
...
for sample_i in range(n_samples):
    ...
    # Spike Encode
    sig = torch.tensor(data).clone()
    data = spikegen.delta(sig, threshold=3)
    ...
    if base_input_current_type == 'constant':
        if dataset_is_spike_encoded:
            base_input = data * base_input_current_gain
        else:
    ...
    conn_lif_lif_enforced_lower_clip = -50

    # conn_lif_lif = torch.normal(
    # 0, 1, size=(reservoir_size, reservoir_size)) * conn_lif_lif_gain

    # conn_lif_lif = src_16.topologies.gen_by_connection_degree(
    #     reservoir_size, reservoir_size, degree=2)

    # topology: positions
    conn_lif_lif_topology = src_16.topologies.gen_rope(
    ...
    synapses = src_16.synapse.Synapse(
    ...
    yi_pred_buffer = src_16.funx.FIFO_buffer(capacity=buffer_capacity)
    yi_pred_logits_buffer = src_16.funx.FIFO_buffer(capacity=buffer_capacity)
    yi_buffer = src_16.funx.FIFO_buffer(capacity=buffer_capacity)
    ...
    for epoch in range(epochs):
        for i, xi_yi in enumerate(zip(data.t(), targets)):
            ...
            for step in range(num_steps):
                ...
                # clip the mem pot
                if conn_lif_lif_enforced_lower_clip is not None:
                    mem1 = mem1.clip(
                        conn_lif_lif_enforced_lower_clip,
                        lif1.threshold)
    ...
    ax = src_16.visualization.generate_raster_plot(spk_hist['filter'].squeeze().T)
    ax.set_xlim(0, 2000)
