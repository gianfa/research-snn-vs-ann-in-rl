...
class ESN(nn.Module):
    ...
    def __init__(self,
            ...
            output_size,
            num_layers=1,
            nonlinearity='tanh',# ['tanh'|'relu'|'id'].
            batch_first=False, # If ``True``, then the input and output 
                # tensors are provided as (batch, seq, feature).
                # Default: ``False``
            leaking_rate=1, # Leaking rate of reservoir's neurons. Default: 1
            spectral_radius=0.9,
            w_ih_scale=1,  # Scale factor for first layer's input weights 
                # (w_ih_l0). It can be a number or a tensor of 
                # size '1 + input_size' and first element is the bias' scale 
                # factor. Default: 1
            lambda_reg=0,
            density=1,
            w_io=False, # 'True': the network uses trainable input-to-output
                # connections. Default: ``False``
        ...
        self.num_layers = num_layers # Number of recurrent layers. Default: 1
        ...
        if w_io:
            ...
            self.readout = nn.Linear(hidden_size * num_layers,
                                     output_size)
