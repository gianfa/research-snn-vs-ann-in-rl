...
for sample_i in range(n_samples):
    ...
    # Spike Encode
    data = spikegen.delta(data, threshold=2)
    ...
    # conn_lif_lif = src_17.topologies.gen_by_connection_degree(
    #     reservoir_size, reservoir_size, degree=2)

    # topology: positions
    conn_lif_lif_topology = src_17.topologies.gen_rope(
    ...
    synapses = src_17.synapse.Synapse(
    ...
    yi_pred_buffer = src_17.funx.FIFO_buffer(capacity=buffer_capacity)
    yi_pred_logits_buffer = src_17.funx.FIFO_buffer(capacity=buffer_capacity)
    yi_buffer = src_17.funx.FIFO_buffer(capacity=buffer_capacity)
    ...
    ax = src_17.visualization.generate_raster_plot(spk_hist['filter'].squeeze().T)
    ax.set_xlim(0, 2000)
