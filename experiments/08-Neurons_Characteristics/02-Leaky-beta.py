...
* Leaky IF . Fixed C, changing beta
...
paramd = {
    'name': 'beta',
    'range': torch.linspace(0.1, 0.5, 6)
}
...
for param in paramd['range']:
    ...
    cur_in = torch.cat((torch.zeros(10), torch.ones(190)*0.6), 0)
    mem = torch.zeros(1)
...
cs = sample_colorscale(
    'burg',
    torch.linspace(0, 1, len(paramd['range'])).tolist())
