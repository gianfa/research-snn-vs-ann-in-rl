"""" Anomaly Detection based on simple LSTM
...
- labels, y: y = {y_1, ..., y_n} = y_i ∈ {0, 1}, with i ∈ {1, n}
...
src11.utils.plot_shifted(shifted_sigs, n=8, offset=3, skip=5)

# Visualize shifted labels
# TODO: make it consistent with plot_shifted()
yy = []
for i in range(8):
    sig_i = 5 + i * 3
    yy.append(shifted_labels[sig_i, :])
yy = np.array(yy)
src11.utils.plot_binary_vectors(yy)
...
model = LSTMAnomalyDetector(input_size, hidden_size, output_size)
