...
EXP_RESULTS_DIR = EXP_DATA_DIR/'exp2/produced_data'
...
fig, ax = plt.subplots()
sns.boxplot(
    data=perf, x='shift', y='r2', hue='type',
...
sns.boxplot(
    data=perf, x='n_STDP_steps', y='r2', hue='type',
    showfliers = False)
plt.show()

sns.boxplot(
    data=perf, x='STDP_scope', y='r2', hue='type',
    showfliers = False)
plt.show()
...
ax.scatter(
    diffs[param].unique(),
    diffs.groupby(param)[f"{metric}_mean"].mean(), s=0.8)
# ax.errorbar(
#     diffs[param],
