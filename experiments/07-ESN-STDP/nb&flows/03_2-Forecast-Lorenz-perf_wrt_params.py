...
# python -m 04-Forecast-Lorenz-perf_wrt_params.py
"""
# %%
...
EXP_RESULTS_DIR = EXP_DATA_DIR / "exp-3/produced_data"
...
for exp_i, exp_pi in enumerate(grid_tuples):
    ...
    for i in range(n_trials):
        ...
        for epoch in range(n_STDP_steps):
            ...
            n_exceding_connections = (
                (new_W != 0).to(torch.int) - (reservoir_connections != 0).to(torch.int)
            ).sum()
    ...
    pickle_save_dict(exp_dir / "perf_hist_stdp.pkl", {"perf_hist_stdp": perf_hist_stdp})
    pickle_save_dict(exp_dir / "W_hist_stdp.pkl", {"W_hist_stdp": W_hist_stdp})
    pickle_save_dict(exp_dir / "W_hist_nonstdp.pkl", {"W_hist_nonstdp": W_hist_nonstdp})
...
for i, exp_dir in enumerate((EXP_RESULTS_DIR).iterdir()):
    ...
    perf_hist_nonstdp = pd.DataFrame(pickle_load(exp_dir / "perf_hist_nonstdp.pkl"))
    perf_hist_nonstdp["example_len"] = exp_params["data"]["example_len"]
    perf_hist_nonstdp["shift"] = exp_params["data"]["shift"]
    perf_hist_nonstdp["n_STDP_steps"] = exp_params["experiment"]["n_STDP_steps"]
    ...
    perf_hist_after_stdp["n_STDP_steps"] = exp_params["experiment"]["n_STDP_steps"]
    ...
    if len(perf) == 0:
        ...
        perf = pd.concat((perf, perf_hist_nonstdp, perf_hist_after_stdp), axis=0)
...
# %%
fig, ax = plt.subplots()
ax.scatter(diffs[param].unique(), diffs.groupby(param)[f"{metric}_mean"].mean(), s=0.8)
# ax.errorbar(
#     diffs[param],
...
ax.plot(
    diffs[param].unique(), diffs.groupby(param)[f"{metric}_mean"].mean(), linewidth=0.6
)
ax.set_xlabel(f"{param}")
ax.set_ylabel(f"{metric} mean")
ax.set_title(r"")
...
diff = np.array(perf_hist_after_stdp["mse"]) - np.array(perf_hist_nonstdp["mse"])
...
diff = np.array(perf_hist_after_stdp["mae"]) - np.array(perf_hist_nonstdp["mae"])
...
stdp_f.plot_most_changing_node_weights(W_hist_stdp[0], n_top_weights=5, ax=axs[0])
