...
for epoch in range(epochs):
    ...
    for i, X_i_Y_i in enumerate(zip(X_train, y_train)):
        ...
        if activate_STDP:
            if epoch > 0 and epoch % 2 == 0: # check ste_to_reset in register
                ...
                print("STDP executed")
        W_hist[epoch].append(model.W.weight)
...
# W_hist_np [=] (epoch, t, W.size, W.size)
W_hist_np = torch.stack([torch.stack(W_hist[i]) for i in range(len(W_hist))]).detach().numpy()
abs_diff = np.abs( np.diff(W_hist_np, axis=0) )
max_indices = np.unravel_index(np.argmax(abs_diff), abs_diff.shape)

# %%
fig, ax = plt.subplots()
i_j_s = [(0, 0), (0, 1), (0, 2)]
for i, j in i_j_s:
    ax.plot(range(W_hist.shape[0]), W_hist[:, i, j], label=f'{i, j}')
