...
sys.path += ['.', '../', '../../', '../../../']
from experimentkit_in.generators.time_series import ts_gen_signal_shifts
import src10
...
class Net(nn.Module):
    ...
    def forward(self, x):
        ...
        for step in range(self.inner_steps):
            ...
            cur2 = self.fc2(mem1) * thresholded_relu(mem1, self.lif1.threshold)
...
cur_in = torch.cat(
   (torch.zeros(10),
    ...
    torch.ones(70) * 0.2),
   0)[:, None]
labels = ((cur_in > 0.2) * (cur_in < 0.8)).float()
...
shifted_sigs = torch.Tensor(
    ts_gen_signal_shifts(cur_in, window_size=window_size, dim=0))
"""Shifted signals
...
X = torch.tensor(shifted_sigs)  # [=] (n_signals, window_size, 1)
y = torch.tensor(shifted_labels)  # [=] (n_signals, window_size, 1)
