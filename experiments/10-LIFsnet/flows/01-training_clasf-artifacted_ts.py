...
class Net(nn.Module):
    def __init__(
        ...
        self.lif1 = snn.Leaky(beta=beta1, threshold=5)
        self.fc2 = nn.Linear(hidden_size, output_size)
        self.lif2 = snn.Leaky(beta=beta2, threshold=5)
    ...
    def forward(self, x):
        ...
        for step in range(self.inner_steps):
            ...
            cur1 = self.fc1(x)  # [=] (bs, hidden_size)

            # mem1 [=] (bs, hidden_size)
            spk1, mem1 = self.lif1(cur1, mem1)

            cur2 = self.fc2(thresholded_relu(mem1, self.lif1.threshold))
...
for epoch in range(num_epochs):
    ...
    for xi, yi in zip(X_train_mb, y_train_mb):
        ...
        # compute loss_val
        loss_val = criterion(
            inner_loop_spk.flatten().to(DTYPE),
            expanded_yi.flatten().to(DTYPE))
...
axs[1].plot(mem_rec['lif1'][range_to_plot].flatten().detach())
axs[1].axhline(net.lif2.threshold, linestyle='-.', color='grey')
axs[1].set_title(f'LIF1 neuron #{var_idx} Membrane Potential')
axs[2].plot(mem_rec['lif2'][range_to_plot].flatten().detach())
axs[2].axhline(net.lif2.threshold, linestyle='-.', color='grey')
axs[2].set_title(f'LIF2 neuron #{var_idx} Membrane Potential')
