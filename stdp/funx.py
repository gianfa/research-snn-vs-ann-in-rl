...
import os  # noqa
import string  # noqa
from typing import Iterable, List, Tuple  # noqa
...
def plot_raster(
    ...
    if isinstance(dt, float):
        ax.set_xlabel('t')
        # ax.xaxis.set_major_locator(mpl.ticker.FixedLocator(np.arange(raster.shape[1]))*dt)
        # ax.set_xticklabels([ti * dt for ti in range(raster.shape[1])])
    else:
        ax.set_xlabel('$t_{step}$')
        ax.xaxis.set_major_locator(mpl.ticker.FixedLocator(range(raster.shape[1])))
